...
<!-- End of side -->
<% 
	ArrayList<PetitionDTO> list = (ArrayList<PetitionDTO>)request.getAttribute("article");
...
<section>
	<article>
	<form action="endPetitionList.fkr" method="post" name="form">
...
<div class="contentListWrap">
	<div class="tab">
		<ul>
			<li><a href="satisfied.fkr">성립된 청원</a></li>
			<li style="background:url('img/sub_line.jpg') no-repeat 2px;"><a href="dissatisfied.fkr">미성립된 청원</a></li>
		</ul>
	</div>
					...
					<div class="contentBox">
						...
						<div class="agreeInfo">
							<ul class="commitList" style="padding: 5px 15px;">
								...
								<%
									if(Integer.parseInt(dto.getAgree_no()) < 1000){
								%>
								 <span>청원만료 및 폐기</span>
								 <%
									}else{
	...
	<article class="pageList">
		...
		%>
		<a href="endPetitionList.fkr?page=<%=nowPage-1 %>" style="text-decoration: none;">[Prev]</a>
		<%
			}
		%>
		...
		%>
		<a href="endPetitionList.fkr?page=<%=i %>" style="text-decoration: none;">[<%=i %>]</a>
		<%
				}
			}
			if(nowPage>=maxPage){
		...
		%>
		<a href="endPetitionList.fkr?page=<%=nowPage+1 %>" style="text-decoration: none;">[Next]</a>
		<%
			}
		%>
